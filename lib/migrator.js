...
class Migrator
{
	...
	async _impersonate(opt)
	{
		...
		{
			const impersonation_token = await this._gitlab.UserImpersonationTokens.add((await this._getGitlabUser({ username: opt.User })).id, `gitlab bot impersonation token for ${opt.User}`, [
				"api", "read_user"
				//"write_user"
				//"read_api",
				//"read_repository",
